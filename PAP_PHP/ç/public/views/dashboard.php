                                                                                ...
                                                                                <?php if ($campo === 'image'): ?>
                                                                                    <td>
                                                                                        <img src="/<?= htmlspecialchars($row[$campo] ?? 'assets/images/users/user_icon.png') ?>" 
                                                                                             alt="user_avatar" width="40" style="height:30px; object-fit:cover;"
...
<div class="modal fade" id="insertPostModal" tabindex="-1">
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <form method="post" id="formInsertPost">
        <div class="modal-body">
          <input type="number" name="id_utilizador" class="form-control mb-2" placeholder="ID do Utilizador" required>
          <textarea name="texto_post" class="form-control mb-2" rows="3" placeholder="Texto do post" required></textarea>
          <input type="text" name="image" class="form-control mb-2" placeholder="URL da Imagem">
        </div>
        <div class="modal-footer">
          <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancelar</button>
          <button type="submit" class="btn btn-primary">Criar</button>
...
<?php foreach($posts as $row): ?>
<div class="modal fade" id="editPostModal-<?= $row['id'] ?>" tabindex="-1">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title">Editar Post #<?= $row['id'] ?></h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
      </div>
      <form method="post" id="formEditPost-<?= $row['id'] ?>">
        <div class="modal-body">
          <textarea name="texto_post" class="form-control mb-2" rows="3" required><?= htmlspecialchars($row['texto_post']) ?></textarea>
          <input type="text" name="image" class="form-control mb-2" placeholder="URL da Imagem" value="<?= htmlspecialchars($row['image'] ?? '') ?>">
        </div>
        <div class="modal-footer">
          <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancelar</button>
          <button type="submit" class="btn btn-primary">Guardar</button>
...
<script>
    ...
    // ---- Criar Post ----
    const formInsertPost = document.getElementById('formInsertPost');
    if (formInsertPost) {
        formInsertPost.addEventListener('submit', async function(e) {
            e.preventDefault();
            const formData = new FormData(this);

            const res  = await fetch('/posts/create', { method: 'POST', body: formData });
            ...
            const novaLinha = `
                <tr>
                    <td>${data.id}</td>
                    <td>${data.username}</td>
                    <td data-campo="image" data-id="${data.id}">
                        ${data.image ? `<img src="${data.image}" style="width:40px;height:40px;object-fit:cover;">` : ''}
                    </td>
                    <td data-campo="texto_post" data-id="${data.id}">${data.texto_post}</td>
                    ...
                    <td>
                        <div class="row">
                            <div class="col-md-12 d-flex">
                                <button class="btn btn-sm btn-warning btnUpdatePost" data-bs-toggle="modal" data-bs-target="#editPostModal-${data.id}" style="width: 80px;">Update</button>
                            </div>
                            <div class="col-md-12 d-flex">
                                <button class="btn btn-sm btn-danger btnDeletePost" style="width: 80px;">Delete</button>
                            </div>
            ...
            document.getElementById('corpoTabelaPostsDB').insertAdjacentHTML('beforeend', novaLinha);
            bootstrap.Modal.getInstance(document.getElementById('insertPostModal')).hide();
            this.reset();
        });
    }

    // ---- Update Post ----
    document.querySelectorAll('[id^="formEditPost-"]').forEach(form => {
        form.addEventListener('submit', async function(e) {
            e.preventDefault();
            const id       = this.id.replace('formEditPost-', '');
            const formData = new FormData(this);

            const res = await fetch('/posts/' + id + '/update', {
                method: 'POST',
                body:   formData
            });
            ...
            if (res.ok) {
                ...
                ['texto_post', 'image'].forEach(campo => {
                    const td = document.querySelector(`[data-campo="${campo}"][data-id="${id}"]`);
                    if (td) td.textContent = formData.get(campo);
                });
            }
